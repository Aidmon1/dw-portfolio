...
## Reconciliation with Unique Identifier in FCRM

The only unique identifier that was in the FCRM model data was the "account_nr" field. RSM used this field as the primary key to find the transactions that were in the core and in the model. The identifier that is used for the core data is the "a_number" field. RSM notes that the core data identifier is insufficient and there is no one to one match.
